/*  =========================================================================
    sphactor_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running libsphactor selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef SPHACTOR_BUILD_DRAFT_API // selftest is still in draft
        else
            sphactor_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("sphactor_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running libsphactor test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef SPHACTOR_BUILD_DRAFT_API // selftest is still in draft
        else
            sphactor_private_selftest (verbose, test->subtest);
#endif // SPHACTOR_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
