...
IF %1.==--help. (
    ...
    ECHO    --without-hello         do not build hello.exe
    ECHO    --without-sphactor_selftest  do not build sphactor_selftest.exe
    GOTO :eof
)
ECHO Configuring libsphactor...
...
ECHO. >> platform.h
ECHO #define SPHACTOR_HAVE_WINDOWS 1>> platform.h
...
IF EXIST "..\..\..\libzmq" (
    ...
    ECHO libsphactor cannot build without libzmq
...
IF EXIST "..\..\..\czmq" (
    ...
    ECHO libsphactor cannot build without czmq
...
:with_draft
    ...
    ECHO #define SPHACTOR_BUILD_DRAFT_API 1>>platform.h
    GOTO end_draft
:no_draft
    ECHO Building without draft API (stable + legacy API)
    ECHO #undef SPHACTOR_BUILD_DRAFT_API 1>>platform.h
:end_draft
ECHO. >> platform.h
